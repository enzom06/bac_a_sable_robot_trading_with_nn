...
lst_x = ['BTTUSDT', 'IOTXUSDT', 'XLMUSDT', 'OMGUSDT', 'BALUSDT', 'DOGEBUSD', 'XTZUSDT', 'ARUSDT', 'DENTUSDT',
         ...
         'COTIUSDT', 'ALPHAUSDT', 'ICPUSDT', 'AXSUSDT', 'ATAUSDT', 'XRPBUSD', 'KNCUSDT', 'RENUSDT', 'C98USDT', 'DASHUSDT',
...
lst_kliine = [Client.KLINE_INTERVAL_1HOUR]
for iaiao in ['XLMUSDT']:

    max_benef_para = -9999999999999999999
    para_total = [0.00, 0.00, 0.0, 0.0]

    for ai in lst_kliine:
        k_interval = ai
        candlesticks = client.get_historical_klines(iaiao, ai, '1 oct, 2021', '14 oct, 2021')
        # candlesticks = client.get_historical_klines('BTCUSDT', Client.KLINE_INTERVAL_2HOUR, '25 mars, 2021', '25 sept, 2021')
...
Pas Accepté:
IOTXUSDT 0.14, 0.29, 0.18, 0.27

Mitigé: (qd il y aura plus de tune dans le compte

XLMUSDT 2h [0.14, 0.0, 0.24, 0.19] grave bon mais il faut vérife les frais

DENTUSDT: [0.0, 0.09, 0.14, 0.15]
ONTUSDT: [0.24, 0.14, 0.15, 0.15] les dernier mois son bon mais avant mitigé car phase de consolidation
ADABUSD: consolidation trop importante donc phase de baisse long
OMGUSDT 4h [0.0, 0.24, 0.13, 0.27] latéralise vers la fin



accepté:
XLMUSDT: 4h [0.09, 0.09, 0.25, 0.26] magnifique
RAYUSDT 2h [0.12, 0.17, 0.2, 0.24]
BTTUSDT: 4h 0.0 0.13 0.17 0.18
DODOUSDT: 4h [0.17, 0.12, 0.24, 0.22] faire gaffe
