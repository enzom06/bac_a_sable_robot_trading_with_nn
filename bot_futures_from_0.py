...
Previous_Date = dt.datetime.today() - dt.timedelta(days=4)
...
candlesticks = client.get_historical_klines(TRADE_SYMBOL, Client.KLINE_INTERVAL_2HOUR, date1, date2)
...
para0 = float(input('entrer le para0:\n--> '))
para1 = float(input('entrer le para1:\n--> '))
para2 = float(input('entrer le para2:\n--> '))
SOCKET = f"wss://fstream.binance.com/ws/{TRADE_SYMBOL.lower()}@kline_2h"
...
print(round(buy_part / ref_price, precision))
...
def dmi_choice(close, lst_adx, lst_adx_pos, lst_adx_neg):
    global in_trade, in_long, in_short, nb_benef, toto_benef, long, short, para0, para1, para2
    # lst_point = []
    ##in_trade = False
    ##in_long = False
    ##in_short = False
    ...
    if not in_trade:
        if lst_adx[-1] > para0:
            ...
            if lst_adx_pos[-1] > lst_adx_neg[-1] and lst_adx_pos[-2] < lst_adx_neg[-2]:
                in_trade = True
                in_long = True
        ...
        if in_long:
            if lst_adx_pos[-1] < lst_adx_neg[-1] and lst_adx_pos[-2] > lst_adx_neg[-2]:
                # in_trade = False
                in_long = False
                if lst_adx[-1] > para1:  # XRP : 18 ,ETH : 19
                    in_short = True
                    short = close
                    a = 2
                else:
            ...
            if lst_adx_pos[-1] > lst_adx_neg[-1] and lst_adx_pos[-2] < lst_adx_neg[-2]:
                ...
                if lst_adx[i] > para2:  # XRP : 18 ,ETH : 19
                    in_long = True
                    long = close
                    a = -2
                else:
...
def real_trade(_candle_high, _candle_low, _candle_close, highs, lows, closes):
    ...
    i = dmi_choice(_candle_close, lst_adx, lst_adx_pos, lst_adx_neg)
...
def on_message(ws, message):
    ...
    if is_candle_closed:
        ...
        print("candle not closed at {} : {}".format(close, dt.datetime.now()))
        opens.append(float(open))
        highs.append(float(high))
        lows.append(float(low))
        closes.append(float(close))
        ...
        # print(closes)
        # print('len closes2', len(closes))
        if len(closes) > (RSI_PERIOD + TIME_TO_TEST) and f == True:
            # np_closes = numpy.array(closes)
            # last_rsi = 0
            # rsi = talib.RSI(np_closes, RSI_PERIOD)
            # print("all rsis calculated so far")
...
prendrre la dernière version sur le rpi
