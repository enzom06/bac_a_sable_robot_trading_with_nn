...
# -- -- -- -- -- -- -- -- -- --
SOCKET = "wss://fstream.binance.com/ws/ethdusdt@kline_1h"
RSI_PERIOD = 14
WILLR_PERIOD = 14
TIME_TO_TEST = 30
TRADE_QUANTITY = 10
...
def real_trade(_candle_high, _candle_low, _candle_close, highs, lows, closes):
    ...
    if i == -1:
        if reserve > max_reserve:
            ...
            buy_part = part
        if reserve >= buy_part:
            order1 = client.order_market_buy(symbol='LTCBUSD',
                                             quoteOrderQty=buy_part)  # il faut faire des test pour savoir si l'ordre market n'est pas lancé
            # print(order1)
            nb_trade += 1
            ...
            if (reserve + _candle_close * temp_token_restant) >= max_reserve:
                if len(lst_total_balance) > 24:
                    ...
                    if lst_total_balance[-1] - lst_total_balance[-24] > 0:
                        ...
                        order2 = client.order_market_sell(symbol='LTCUBSD', quantity=lst_buy[0])
                        ...
                        order2 = client.order_market_sell(symbol='LTCBUSD', quantity=lst_buy[0])
                        reserve += float(order2['cummulativeQuoteQty'])
                        all_fees += float(order2['fills'][0]['commission'])
    ...
    temp_account = client.get_account()

    reserve = temp_account['balances']
    for i in reserve:
        if i['asset'] == 'BUSD':
            reserve = i['free']
            # print(reserve)
...
def on_message(ws, message):
    ...
    if is_candle_closed:
        ...
        if len(closes) > RSI_PERIOD + TIME_TO_TEST:
            ...
            real_trade(float(high), float(low), float(close), highs[::-1], lows[::-1], closes[::-1])
            print('-- ' * 20)
...
les benef du bot sont encore dans lea reserve
